...
#         self.dropout = torch.nn.Dropout(p=0.5, inplace=False)
        if self.pca:
            ...
            self.compress_activations_conv = nn.Conv3d(in_channels=128, out_channels=48, kernel_size=(1, 8, 8), stride=(1, 2, 2), padding=(1, 4, 4))
...
#                                         requires_grad=True)
            ...
            self.fc1 = nn.Linear(575280, 1000)
...
#                 x[:, i, :, :, :] = activations[:, i, :, :, :] * self.feature_weights[i]
            ...
            x = F.relu(self.compress_activations_conv(activations))#.view(batch_size, channel_size, time_size,-1)
...
#             frozen_sparse.load_state_dict(sparse_param['model_state_dict'])
        ...
        if args.use_clips:
            train_loader, test_loader = load_bamc_clips(batch_size, 0.8, sparse_model=frozen_sparse, device=device, num_frames=5, seed=args.seed + i_fold)
        else:
...
#                         frozen_sparse.get_output_shape(example_data[1]))
    ...
    print("Final accuracy={:.2f}".format(np.array(all_errors).mean()))